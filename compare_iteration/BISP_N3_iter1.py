...
for i in range(np.shape(linux_pre)[0]):
    for j in range(np.shape(linux_pre)[1]):
        if mysql_std[j] != 0:
           linux_pre[i][j] = (linux_x[i][j] - mysql_mean[j]) / mysql_std[j]
        else:
           linux_pre[i][j] = linux_x[i][j] - mysql_mean[j]
...
def init_model(method, base_estimator, n_estimators):
    ...
    if method == 'SPEnsemble':
        model = SelfPacedEnsemble(base_estimator=base_estimator, n_estimators=n_estimators, random_state=13)
...
def SPEtest(Xs, Ys, Xt, Yt):
    ...
    #Xs_new, Xt_new = tca.fit(mysql_pre, linux_pre)
    runs = 1
    ...
    for method in method_list:
            ...
            with trange(runs) as t:
                for _ in t:
                    model = init_model(
                        ...
                        base_estimator=RandomForestClassifier(n_estimators=10, max_depth=1, random_state=3),
        ...
        print('------------------------------')
        print('Metrics:')
        df_scores = pd.DataFrame(scores, columns=['AUC', 'F1', 'G-mean', 'MCC'])
    ...
    return y_pred_b, AUC, F1, gm, mcc, pdx, pf, bal
...
class IDSA:
    ...
    def isda(self, Xs, Ys, Xt, Yt):
        ...
        y_pred, AUC, F1, gm, mcc, pdx, pf, bal= SPEtest(Xs_new, XX_y, Yt_new, Yt)
        #print(auc)
        #print(f1)
        ...
        return PD, PF, Bal, AUC, F1, gm, mcc, pdx, pf, bal, y_pred
...
class BDA:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ...
        for t in range(self.T):
            N = 0
            M0 = e * e.T * C
            if Y_tar_pseudo is not None and len(Y_tar_pseudo) == nt and len(Y_tar_pseudo[np.where(Y_tar_pseudo == 1)]) != 0 and len(Y_tar_pseudo[np.where(Y_tar_pseudo == 0)]) != 0:
                for c in range(2):
                    e = np.zeros((n, 1))
            ...
            dsa = IDSA(type='lr')
            PD, PF, Bal, AUC, F1, gm, mcc, pdx, pf, bal, y_pred = dsa.isda(Xs_new, Ys, Xt_new, Yt)
            Y_tar_pseudo = y_pred

            # jj = 5
            # oldWb = xlrd.open_workbook('D:\\实验数据\\iteration.xls')
            # newWb = copy(oldWb)
            # newWs = newWb.get_sheet('Sheet1')
            # newWs.write(18 + t, jj, PD)
            # newWs.write(65 + t, jj, PF)
            # newWs.write(112 + t, jj, Bal)
            # newWs.write(159 + t, jj, AUC)
            # newWb.save('D:\\实验数据\\iteration.xls')
        ...
        return PD, PF, Bal, AUC, F1, gm, mcc, pdx, pf, bal
...
if __name__ == '__main__':
    ...
    # for i in range(10):
    Xs, Ys, Xt, Yt = mysql_pre, mysql_y, linux_pre, linux_y

    bda = BDA(kernel_type='primal', dim=10, lamb=0.9, mu=0.1, mode='BDA', gamma=1, estimate_mu=True)
    PD, PF, Bal, AUC, F1, gm, mcc, pdx, pf, bal = bda.fit_predict(Xs, Ys, Xt, Yt)
...
# print('auc', np.mean(AUC1))
    # print('mcc', np.mean(mcc1))
    # print('f1', np.mean(f11))
    # print('gm1', np.mean(gm1))
    # print(np.mean(pd1))
    # print(np.mean(pf1))
    # print(np.mean(bal1))
    # print(np.mean(Bal1))
