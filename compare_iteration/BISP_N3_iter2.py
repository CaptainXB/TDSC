...
for i in range(np.shape(netbsd_pre)[0]):
    for j in range(np.shape(netbsd_pre)[1]):
        if linux_std[j] != 0:
           netbsd_pre[i][j] = (netbsd_x[i][j] - linux_mean[j]) / linux_std[j]
        else:
           netbsd_pre[i][j] = netbsd_x[i][j] - linux_mean[j]
...
class BDA:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ...
        for t in range(self.T):
            ...
            newWs.write(3 + t, jj, '{:.4f}'.format(PD))
            newWs.write(50 + t, jj, '{:.4f}'.format(PF))
            newWs.write(97 + t, jj, '{:.4f}'.format(Bal))
            newWs.write(144 + t, jj, '{:.4f}'.format(AUC))
...
if __name__ == '__main__':
    ...
    # for i in range(10):
    Xs, Ys, Xt, Yt = linux_pre, linux_y, netbsd_pre, netbsd_y

    bda = BDA(kernel_type='primal', dim=10, lamb=0.9, mu=0.1, mode='BDA', gamma=1, estimate_mu=False)
