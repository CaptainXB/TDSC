...
class SelfPacedEnsemble():
    ...
    def _self_paced_under_sampling(self,
        ...
        # Else allocate majority samples into k hardness bins
            ...
            for i_bins in range(self._k_bins):
                ...
                # print(sum(hardness[idx]))
            # print(np.shape(ave_contributions))
            # Update self-paced factor alpha
            # alpha = np.tan(np.pi*0.5*(i_estimator/(self._n_estimators-1)))
            alpha = np.tan(np.pi * 0.5 * ((9 - i_estimator) / (self._n_estimators - 2))) / 10
            # alpha = np.tan(np.pi * 0.5 * (9 / (self._n_estimators - 1)))
            # print(alpha)
            # alpha = float(0.1)
            ...
            n_sample_bins = n_sample_bins.astype(int)
            # print(n_sample_bins)
            # n_sample_bins[9] = n_sample_bins[0] + n_sample_bins[9]

            # Perform self-paced under-sampling
            sampled_bins = []
            # for i in range(10):
            #   print(i)
            #  print(np.shape(bins[i]))
            center_sample_negative = np.mean(X_min, axis=0)
            center_sample_bins = []
            for i_bins in range(self._k_bins):
                center_sample_bins.append(np.mean(bins[i_bins], axis=0))
            # print(np.shape(center_sample_bins))
            for i_bins in range(self._k_bins):
                if min(len(bins[i_bins]), n_sample_bins[i_bins]) > 0:
                    np.random.seed(self._random_state)
                    '''
                    #calculate the euclidean distance and select data from each bin
                    #idx = []
                    distance = []

                    for i_sample in range(len(bins[i_bins])):
                        distance.append([i_sample, np.sqrt(sum(np.power((bins[i_bins][i_sample] - center_sample_bins[i_bins]), 2)))])
                        #distance.append([i_sample, np.sqrt(sum(np.power((bins[i_bins][i_sample] - center_sample_negative), 2)))])
                    ...
                    #print(n_sample_bins)
                    #print(n_sample_bins[i_bins])
                    episode_select = int(len(distance) / min(len(bins[i_bins]), n_sample_bins[i_bins]))
                    #for i_x in range(int(min(len(bins[i_bins]), n_sample_bins[i_bins]) / 2)):
                     #   idx.append(distance[i_x][0])
                    for i_x in range(int(min(len(bins[i_bins]), n_sample_bins[i_bins]))):
                        idx.append(distance[np.random.randint(i_x * episode_select, (i_x + 1) * episode_select)][0])
                        #idx.append(distance[i_x * episode_select][0])
                        #idx.append(distance[int(len(distance)/2)-i_x][0])
                        #idx.append(distance[len(distance)-i_x-1][0])
                    #print(idx)
                    #print(np.shape(distance))
    ...
    def fit(self, X, y, label_maj=0, label_min=1):
        ...
        for i_estimator in range(1, self._n_estimators):
            ...
            y_pred_org = np.zeros((X_maj.shape[0], 2))
            for model in self.estimators_:
                y_pred_org += model.predict_proba(X_maj)
            self._y_pred_maj = (y_pred_org / n_clf)[:, 1]
    ...
    def predict_proba(self, X):
        ...
        for model in self.estimators_:
            y_pred = y_pred + model.predict_proba(X)
        y_pred = y_pred/len(self.estimators_)
        # y_pred = np.array(
