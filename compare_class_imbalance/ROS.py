...
mysql = np.loadtxt('F:\\dataset\\data\\mysql.csv', delimiter=',', skiprows=1)
mysql_x = np.delete(mysql, -1, axis=1)
mysql_y = mysql[:, -1]
# print(mysql_y)

linux = np.loadtxt('F:\\dataset\\data\\linux.csv', delimiter=',', skiprows=1)
...
def SMOTE_RkNN(train_x_smo, train_y_smo):
    nn = NearestNeighbors(algorithm='auto', n_neighbors=int(math.sqrt(train_x_smo.shape[0])) + 1)
    nn.fit(train_x_smo)
    distance = nn.kneighbors(train_x_smo)[1]
    min_density = np.zeros((train_x_smo.shape[0], 1))
    maj_density = np.zeros((train_x_smo.shape[0], 1))
    for i in range(train_x_smo.shape[0]):
        for j in range(1, distance.shape[1]):
            if train_y_smo[i] == 1:
                min_density[distance[i][j]] += 1
            else:
                maj_density[distance[i][j]] += 1

    min_sum = np.sum(min_density)
    maj_sum = np.sum(maj_density)

    drop_list = []
    alph = 2
    for i in range(train_x_smo.shape[0]):
        if maj_density[i][0] / maj_sum >= alph * min_density[i][0] / min_sum and min_density[i][0] != 0 and train_y_smo[
            i] == 1:
            drop_list.append(i)
        elif min_density[i][0] / min_sum >= alph * maj_density[i][0] / maj_sum and maj_density[i][0] != 0 and \
                train_y_smo[i] == 0:
            drop_list.append(i)


    train_x_clean = np.delete(train_x_smo, drop_list, axis=0)
    train_y_clean = np.delete(train_y_smo, drop_list)

    return train_x_clean, train_y_clean
...
class BDA:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ...
        for t in range(self.T):
            ...
            x_resampled, y_resampled = SMOTE_RkNN(x_resampled, y_resampled)
...
if __name__ == '__main__':
    ...
    for i in range(10):
        Xt, Yt, Xs, Ys = mysql_pre, mysql_y, linux_pre, linux_y
